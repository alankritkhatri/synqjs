...
async function runTests() {
  ...
  await test("RACE CONDITION DETECTED: Job ID collision", async () => {
    ...
    console.log(
      `   ⚠️  Race condition: Multiple jobs got same timestamp-based ID`
    );

    // This test expects the race condition to exist
    assert(
      queueLength < 10,
      `Race condition confirmed: ${queueLength} unique jobs created from 10 submissions`
    );
  });
  ...
  await test("Job processing should be atomic", async () => {
    ...
    assert(
      jobsReceived.length === 1,
      `Expected 1 job, got ${jobsReceived.length}`
    );
  ...
  await test("System performance under concurrent load", async () => {
    ...
    console.log(
      `   📊 Average: ${(duration / queueLength).toFixed(2)}ms per job`
    );
